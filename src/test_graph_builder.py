...
from src.models.graph import GraphBuildResult, GraphTriple
...
def test_write_triples_uses_parameterized_merge() -> None:
    ...
    # 4 index setup queries + 1 triple query per write_triples call
    # Each call runs setup_vector_indices (4 queries) then 1 triple query
    assert len(session.queries) == 10
    
    # The actual triple MERGE is the 5th and 10th query in the sequence
    triple_query_1 = session.queries[4]
    assert "MERGE" in triple_query_1[0]
    assert triple_query_1[1]["product_sku"] == "SKU-1"
    assert triple_query_1[1]["mechanism_embedding"] == [1.0, 2.0]
    assert triple_query_1[1]["source_pmid"] == "23853635"
    assert "SUPPORTED_BY" in triple_query_1[0]
